...
# Removed unused returned variable from Cell_segmentation function
...
def Cell_Segmentation(imp, SegmentChoise):
	# duplicate input image
	DAPI = imp.duplicate();
	IJ.run(DAPI, "8-bit", "");
	if SegmentChoise == True:
		gd = GenericDialog("Adjust the segmentation parameters:")
		gd.addNumericField("Rolling", 50, 1)
		gd.addNumericField("Sigma", 1, 1)
		if gd.wasCanceled(): 
			gd = GenericDialog("Error") 
			gd.addMessage("You wanted to adjust your segmentation paramaters, but you did not. Run the script again.")
			gd.hideCancelButton()		
			gd.showDialog()
			return 0   
		elif gd.wasOKed():
			rolling = gd.getNextNumber()
			sigma = gd.getNextNumber()
		else:
			rolling = 50
			sigma = 1
	# run cell segmentation on DAPI channel. Method: Default
	IJ.run(DAPI, "Subtract Background...", rolling); #TADY TO MUSIM DAT DO TE FRAZE!!!!!
	IJ.run(DAPI, "Gaussian Blur...", sigma);  #TADY TO MUSIM DAT DO TE FRAZE!!!!!
...
def main():
	...
	gd.addCheckbox("I want to adjust the segmentation parameters.", False)
	gd.showDialog()
	...
	if gd.wasCanceled(): 
		...
		global RoiManHave
		SegmentChoise = gd.getNextBoolean()
		global SegmentChoise
	...
	for filename in workDir:
		...
		# Process DAPI channel - make the segmentation and select the cell to be analysed
			if filename1.endswith('.czi'):					
				Cell_Map = Cell_Segmentation(imps[1],SegmentChoise)
			else:
				Cell_Map = Cell_Segmentation(imps[0],SegmentChoise)
